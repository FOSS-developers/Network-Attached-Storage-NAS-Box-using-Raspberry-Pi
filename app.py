...
def signup():
    ...
    if request.method == 'POST':
        ...
        query = "SELECT * FROM `login` WHERE `username` = %s AND `password` = " \
                "%s"
...
def share():
    ...
    if request.method == 'POST':
        if 'username' in session:
            ...
            cursor.execute("select directory,share_name from samba")
            data = cursor.fetchall()
            return render_template('samba.html', username=username,
...
def start_web():
    ...
    if request.method == 'POST':
        if 'username' in session:
            ...
            cursor = mariadb_connection.cursor()
            cursor.execute("insert into web(project_name,url) values(%s,%s)",
                           (project_name, url))
            ...
            cursor.execute("select project_name,url,port from web")
            data = cursor.fetchall()
            return render_template('web.html', username=username, result=result,
                                   data=data)
        error = "Login first"
        return render_template('home.html', error=error)
    error = "Login first"
    ...
    if request.method == 'GET':
        if 'username' in session:
            ...
            return render_template('browse.html', username=username,
                                   files=files, files_number=files_number)
        error = "Login first"
        return render_template('home.html', error=error)
    if request.method == 'POST':
        if 'username' in session:
...
def monitor():
    now = datetime.datetime.now()
    timeString = now.strftime("%Y-%m-%d %H:%M")
    # ip= os.popen('ip addr show wlp19s0').read().split("inet ")[1].split(
    # "/")[0]   your_ip = request.remote_addr
    model = os.popen(
        "cat /proc/cpuinfo  | grep 'model name\|Hardware\|Serial' | "
        "uniq").read()
    ...
    temp = a / float(1000)
    release = os.popen(
        "cat /etc/*-release | grep PRETTY_NAME | cut -d= -f2").read()
    cpu_use = os.popen(
        "top -b -n2 | grep 'Cpu(s)'|tail -n 1 | awk '{print $2 + $4 }'").read()
...
def start_ftp():
    ...
    if request.method == 'POST':
        if 'username' in session:
            ...
            cursor = mariadb_connection.cursor()
            cursor.execute("insert into ftp(username,directory) values(%s,%s)",
                           (username, f.filename))
            ...
            cursor.execute("select username,directory from ftp")
            data = cursor.fetchall()
            return render_template('ftp.html', username=username, result=result,
                                   data=data)
        error = "Login first"
        return render_template('home.html', error=error)
    error = "Login first"
...
def contact():
    ...
    if 'username' in session:
        ...
        if request.method == 'POST':
            ...
            total = "Name : " + name + "\nEmail : " + email + "\nMessage : " + message
            msg = Message('Help-NAS Box using Raspberry Pi',
                          sender='ntkathole@gmail.com',
                          recipients=['nikhilkathole2683@gmail.com'])
            msg.body = total
            mail.send(msg)
            msg = Message('Admin-NAS Box using Raspberry Pi',
                          sender='ntkathole@gmail.com', recipients=[email])
